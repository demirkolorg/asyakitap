...
interface SummariesClientProps {
    ...
    booksWithoutTortu: { id: string; title: string; coverUrl: string | null }[]
}
...
export default function SummariesClient({ booksWithTortu, totalBookCount, booksWithoutTortu }: SummariesClientProps) {
    ...
    const stats = useMemo(() => {
        ...
        return {
            ...
            coverage: totalBookCount > 0 ? Math.round((booksWithTortu.length / totalBookCount) * 100) : 0,
        }
    }, [booksWithTortu, totalBookCount])
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col gap-2">
                <h1 className="text-3xl md:text-4xl font-black tracking-tight">Tortular</h1>
                <p className="text-muted-foreground">
                    Kitaplardan aklında kalanlar, özet notların.
                </p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-card rounded-xl p-4 border border-border/50 flex flex-col gap-1 relative overflow-hidden group hover:border-border transition-colors">
                    ...
                    <span className="text-muted-foreground text-sm font-medium z-10">Tortu Yazılan</span>
                ...
                <div className="bg-card rounded-xl p-4 border border-border/50 flex flex-col gap-1 relative overflow-hidden group hover:border-primary/50 transition-colors">
                    ...
                    <span className="text-3xl font-black text-primary z-10">{stats.totalWords.toLocaleString()}</span>
                </div>
                <div className="bg-card rounded-xl p-4 border border-border/50 flex flex-col gap-1 relative overflow-hidden group hover:border-green-500/50 transition-colors">
                    ...
                    <span className="text-muted-foreground text-sm font-medium z-10">Tamamlanan</span>
            ...
            <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center bg-card/50 p-4 rounded-xl border border-border/50">
                ...
                {booksWithoutTortu.length > 0 && (
                    <div className="text-sm text-muted-foreground">
                        <span className="text-orange-500 font-semibold">{booksWithoutTortu.length}</span> kitap tortu bekliyor
                    </div>
                )}
            </div>
